"""
evaluate_yolo_knife_boxes.py

BOX-LEVEL EVALUATION FOR KNIFE DETECTION (with confusion matrix + bar chart)

Pipeline:
1) Run YOLOv8 on the chosen split (valid/test) and save predictions as YOLO .txt
   files AND annotated images.
2) Compare prediction .txt files with GT .txt files at the BOX level, for a
   single class: KNIFE_CLASS_ID (usually 0 for 'knife').

Definitions (box-level):

- GT boxes: all boxes in GT label files with class == KNIFE_CLASS_ID
- Pred boxes: all boxes in prediction txt files with class == KNIFE_CLASS_ID

- A predicted box is a TRUE POSITIVE (TP) if:
    * It has class == KNIFE_CLASS_ID, and
...
Bar chart:
- "Detected (TP)" vs "Missed (FN)" for GT knife boxes.
...
# Index of "knife" in your YOLO labels (for names: ['knife'], this is 0)
KNIFE_CLASS_ID = 0

# Base dir for your desk-14 computer vision project
BASE_DIR = r"C:\Users\lenovo\Desktop\Knife"

# 1) Trained weights  ---> CHANGE THIS TO YOUR .pt FILE NAME
WEIGHTS_PATH = r"C:\Users\lenovo\Desktop\Knife\runs\train_20251126_205247\weights\best.pt"

# 2) Dataset root + which split to evaluate on ("train", "valid", or "test")
DATASET_ROOT = os.path.join(BASE_DIR, "Dataset")
SPLIT = "test"   # or "valid" if you want validation set
...
PRED_PROJECT_DIR = os.path.join(DATASET_ROOT, "predictions_knife")
...
def evaluate_box_level(labels_dir: str, pred_labels_dir: str, image_filenames):
    """
    For each image:
      - Load GT boxes (knife class) from labels_dir
      - Load predicted boxes (knife class) from pred_labels_dir
    ...
    """
    print("\n========== BOX-LEVEL EVALUATION (knife class only) ==========")
    total_tp = 0
    total_fp = 0
    total_fn = 0
    ...
    for img_name in image_filenames:
        ...
        gt_boxes = load_yolo_boxes(gt_txt_path, KNIFE_CLASS_ID)
        pred_boxes = load_yolo_boxes(pred_txt_path, KNIFE_CLASS_ID)

        tp, fp, fn = match_boxes_per_image(gt_boxes, pred_boxes,
    ...
    print(f"\nTotal GT boxes (knife): {total_tp + total_fn}")
    print(f"Total predicted boxes (knife): {total_tp + total_fp}")
    ...
    print("Row 1: GT = GT knife    -> [FN, TP]")

    disp = ConfusionMatrixDisplay(
        confusion_matrix=cm,
        display_labels=["No GT box", "GT knife"],
    )
    disp.plot(cmap="Blues")
    plt.title("Box-level Confusion (Knife vs No-GT-Match)")
    plt.tight_layout()
    plt.show()
    ...
    plt.title("GT Knife Boxes: Detected vs Missed")
...
def main():
    print("Loading knife detector model...")
